// LIVE CERT — /cash-out/new rebuilt 1:1 with moysklad's «Расходный ордер» («+ Расход»
// 2nd form). Meta labels; Статья расходов + Основание + Без закрывающих документов;
// inline ref; owner popover; save → /cash-out/<uuid>. 0 console err.
...
page.on('response', async (r) => {
  if (r.request().method() === 'POST' && r.url().includes('/cash-out')) {
    let body = '';
    try {
...
try {
  ...
  await page.goto(`${BASE}/cash-out/new`, { waitUntil: 'domcontentloaded' });
  await page.locator(sel('cash-out-new-page')).waitFor({ timeout: 120000 });
  await page.waitForTimeout(1500);
  ok('/cash-out/new renders');
  await page.screenshot({ path: resolve(OUT, 'our-cout-01-full.png') });
  await page.screenshot({ path: resolve(OUT, 'our-cout-02-meta.png'), clip: { x: 0, y: 90, width: 940, height: 480 } });

  const body = (await page.locator('body').innerText()).replace(/\s+/g, ' ');
  for (const label of ['Организация', 'Контрагент', 'Баланс', 'Договор', 'Сумма', 'Включая НДС', 'Проект', 'Канал продаж', 'Статья расходов', 'Основание', 'Валюта документа', 'Без закрывающих документов']) {
    if (body.includes(label)) ok(`meta «${label}» present`);
    else bad(`meta «${label}» MISSING`);
  }
  for (const absent of ['Входящий номер', 'Счёт организации', 'Внешний код']) {
  ...
  ok('Сумма filled');
  await page.screenshot({ path: resolve(OUT, 'our-cout-03-filled.png'), clip: { x: 0, y: 90, width: 940, height: 480 } });

  await page.locator('button:has-text("Сохранить")').first().click().catch(() => {});
  const saved = await page
    .waitForURL((u) => /\/cash-out\/[0-9a-f-]{36}/.test(u.pathname), { timeout: 30000 })
    .then(() => true)
    .catch(() => false);
