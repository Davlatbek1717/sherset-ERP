...
/**
 * /settings/sales-return-statuses — moysklad «Настроить статусы» parity
 * (for sales-returns / Возвраты покупателей).
 *
 * Account-wide editable statuses (State rows, entityType="salesreturn") that
 ...
 * /settings/purchase-return-statuses 1:1 (same State REST surface).
 *
...
// matches the customer-order / purchase-return palette so all status sets read identically.
...
export default function SalesReturnStatusesPage() {
  const t = useTranslations('sales_return_statuses');
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`sales-return-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`sales-return-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        testId="sales-return-statuses-page"
        title={t('title')}
        subtitle={t('subtitle')}
        moyskladToolbar
        onCreate={openCreate}
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `sales-return-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
      ...
      <Modal
        ...
        widthClass="w-[480px]"
        testId="sales-return-status-modal"
        footer={
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label
              htmlFor="sales-return-status-name"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('name')}
            </label>
            <Input
              id="sales-return-status-name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              data-test-id="sales-return-status-name"
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`sales-return-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              htmlFor="sales-return-status-position"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('position')}
            </label>
            <Input
              id="sales-return-status-position"
              type="number"
              value={position}
              onChange={(e) => setPosition(Number(e.target.value) || 0)}
              data-test-id="sales-return-status-position"
            />
          </div>
          ...
          {error && (
            <div
              ...
              data-test-id="sales-return-status-error"
            >
              {error}
            </div>
          )}
