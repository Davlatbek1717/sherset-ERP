...
/**
 * /settings/demand-statuses — moysklad «Настроить статусы» parity (for Отгрузки).
 *
 * Account-wide editable statuses (State rows, entityType="demand") that drive
 * the coloured «Статус» pill on the demand list (column + «Статус ▾» toolbar
 * dropdown + «Статус» filter). Until the admin creates statuses here the demand's
 * «Статус» cell stays a grey «Status» placeholder (the FSM `state` + «Проведено»
 * are separate). Single-page CRUD with an inline create/edit modal — name +
 * colour + position — mirroring /settings/supply-statuses 1:1 (same State REST).
 *
...
// matches the supply/customer-order palette so all status sets read identically.
...
export default function DemandStatusesPage() {
  const t = useTranslations('demand_statuses');
  ...
  const upsertMut = useMutation({
    ...
    onSuccess: () => {
      // Invalidate the whole `states` key so the demand toolbar dropdown + column
      // + «Статус» filter pick up the change too.
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`demand-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`demand-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `demand-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label
              htmlFor="demand-status-name"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('name')}
            </label>
            <Input
              id="demand-status-name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              data-test-id="demand-status-name"
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`demand-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              htmlFor="demand-status-position"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('position')}
            </label>
            <Input
              id="demand-status-position"
              type="number"
              value={position}
              onChange={(e) => setPosition(Number(e.target.value) || 0)}
              data-test-id="demand-status-position"
            />
          </div>
          ...
          {error && (
            <div
              ...
              data-test-id="demand-status-error"
            >
              {error}
            </div>
          )}
