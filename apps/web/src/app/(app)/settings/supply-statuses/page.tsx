...
/**
 * /settings/supply-statuses — moysklad «Настроить статусы» parity
 * (for supplies / Приёмки).
 *
 * Account-wide editable statuses (State rows, entityType="supply") that drive
 * the coloured «Статус» dropdown on the supply list toolbar + detail header.
 * Until the admin creates statuses here the supply's «Статус» pill stays a grey
...
export default function SupplyStatusesPage() {
  const t = useTranslations('supply_statuses');
  ...
  const upsertMut = useMutation({
    ...
    onSuccess: () => {
      // Invalidate the whole `states` key so the supply toolbar dropdown picks up
      // the change too.
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`supply-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`supply-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `supply-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label
              htmlFor="supply-status-name"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('name')}
            </label>
            <Input
              id="supply-status-name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              data-test-id="supply-status-name"
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`supply-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              htmlFor="supply-status-position"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('position')}
            </label>
            <Input
              id="supply-status-position"
              type="number"
              value={position}
              onChange={(e) => setPosition(Number(e.target.value) || 0)}
              data-test-id="supply-status-position"
            />
          </div>
          ...
          {error && (
            <div
              ...
              data-test-id="supply-status-error"
            >
              {error}
            </div>
          )}
