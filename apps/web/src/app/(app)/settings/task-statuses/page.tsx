...
/**
 * /settings/task-statuses — moysklad «Тип задачи» management (task states).
 *
 * The account-wide editable list (State rows, entityType="task") that drives the
 * coloured «Тип задачи» dropdown on the task-create slide-over + the card's Задачи
 * list (e.g. Вазифа / Текшириш / Мухим иш). moysklad's «Тип задачи» IS the task's
 * `state`, so this is the same /states CRUD the order/counterparty status pages use
 * — just entityType="task". Reuses the `task_types` i18n namespace (same labels).
 *
 * Supersedes /settings/task-types (which manages the legacy, now-dormant TaskType
 * table). The /states list returns {items, nextCursor} (no total/search/sort), so
 * search is applied client-side; the list is small.
 */
...
// Includes the live moysklad account's task-state hues (lime/blue/red — Вазифа/
// Текшириш/Мухим иш) so user-created types match what moysklad already shows.
...
export default function TaskStatusesPage() {
  ...
  // /states has no server-side search; filter the (small) list locally.
  ...
  const upsertMut = useMutation({
    mutationFn: async () => {
      ...
      return api.post('/states', { entityType: ENTITY_TYPE, name: name.trim(), color, position });
    },
    onSuccess: () => {
      // Invalidate the whole `states` key so the task-create dropdown picks it up too.
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`task-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`task-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `task-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`task-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input
              ...
              data-test-id="task-status-position"
            />
          </div>
          ...
          {error && (
            <div
              ...
              data-test-id="task-status-error"
            >
              {error}
            </div>
          )}
