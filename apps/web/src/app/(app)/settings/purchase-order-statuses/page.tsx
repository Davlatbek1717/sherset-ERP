...
/**
 * /settings/purchase-order-statuses — moysklad «Настроить статусы» parity
 * (for purchase orders / Заказы поставщикам).
 *
 * Account-wide editable statuses (State rows, entityType="purchaseorder") that
 * drive the coloured «Статус» dropdown on the purchase-order detail header.
 * Until the admin creates statuses here the order's «Статус» pill stays a grey
 ...
 * /settings/customer-order-statuses 1:1 (same State REST surface).
 *
...
// matches the customer-order palette so both status sets read identically.
...
export default function PurchaseOrderStatusesPage() {
  const t = useTranslations('purchase_order_statuses');
  ...
  const upsertMut = useMutation({
    ...
    onSuccess: () => {
      // Invalidate the whole `states` key so the PO detail dropdown picks up the
      // change too.
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`po-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`po-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        testId="purchase-order-statuses-page"
        title={t('title')}
        subtitle={t('subtitle')}
        moyskladToolbar
        onCreate={openCreate}
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `po-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
      ...
      >
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`po-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              htmlFor="po-status-position"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('position')}
            </label>
            <Input
              id="po-status-position"
              type="number"
              value={position}
              onChange={(e) => setPosition(Number(e.target.value) || 0)}
              data-test-id="po-status-position"
            />
          </div>
