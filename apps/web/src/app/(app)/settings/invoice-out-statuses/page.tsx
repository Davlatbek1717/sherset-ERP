...
/**
 * /settings/invoice-out-statuses — moysklad «Настроить статусы» parity
 * (for customer invoices / Счета покупателям).
 *
 * Account-wide editable statuses (State rows, entityType="invoiceout") that drive
 * the coloured «Статус» dropdown on the invoice-out editors' header pill + list column.
 * Until the admin creates statuses here the invoice's «Статус» pill stays a grey
 * «Статус» (the FSM `state` + «Проведено» are separate). Single-page CRUD with an
 * inline create/edit modal — name + colour + position — mirroring
 * /settings/purchase-order-statuses 1:1 (same State REST surface).
 *
...
export default function InvoiceOutStatusesPage() {
  const t = useTranslations('invoice_out_statuses');
  ...
  const upsertMut = useMutation({
    ...
    onSuccess: () => {
      // Invalidate the whole `states` key so the invoice-out editors pick up
      // the change too.
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`invoice-out-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`invoice-out-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        testId="invoice-out-statuses-page"
        title={t('title')}
        subtitle={t('subtitle')}
        moyskladToolbar
        onCreate={openCreate}
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `invoice-out-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
      ...
      <Modal
        ...
        widthClass="w-[480px]"
        testId="invoice-out-status-modal"
        footer={
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label
              htmlFor="invoice-out-status-name"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('name')}
            </label>
            <Input
              id="invoice-out-status-name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              data-test-id="invoice-out-status-name"
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`invoice-out-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              htmlFor="invoice-out-status-position"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('position')}
            </label>
            <Input
              id="invoice-out-status-position"
              type="number"
              value={position}
              onChange={(e) => setPosition(Number(e.target.value) || 0)}
              data-test-id="invoice-out-status-position"
            />
          </div>
          ...
          {error && (
            <div
              ...
              data-test-id="invoice-out-status-error"
            >
              {error}
            </div>
          )}
