...
export default function EditSmenaPage() {
  ...
  const { data: orgs } = useQuery<{ items: Option[] }>({
  ...
  const submit = (ev: React.FormEvent) => {
    ev.preventDefault();
    if (!name.trim()) {
      setError('Smena nomi kiritilishi shart');
      return;
    }
    if (!scheduleId) {
      setError('Ish vaqti tanlanishi shart');
      return;
    }
    if (!organizationId) {
      setError('Tashkilot tanlanishi shart');
      return;
    }
  ...
  if (isLoading)
    return <div className="p-6 text-sm text-[var(--ms-text-muted)]">Yuklanmoqda...</div>;

  return (
    <div className="p-6 max-w-xl">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-[var(--ms-text-primary)]">
          {smena?.name ?? 'Smena'}
        </h1>
        <Button
          variant="destructive"
          size="sm"
          onClick={() => {
            if (confirm("Smena o'chirilsinmi?")) deleteMut.mutate();
          }}
      ...
      <form onSubmit={submit} className="flex flex-col gap-4">
        <div>
          <label htmlFor="se-name" className="block text-sm font-medium mb-1">
            Nomi <span className="text-[var(--ms-text-destructive)]">*</span>
          </label>
          <Input id="se-name" value={name} onChange={(e) => setName(e.target.value)} />
        </div>

        <div>
          <label htmlFor="se-schedule" className="block text-sm font-medium mb-1">
            Ish vaqti <span className="text-[var(--ms-text-destructive)]">*</span>
          </label>
          <NativeSelect
            id="se-schedule"
            value={scheduleId}
        ...
        <div>
          <label htmlFor="se-org" className="block text-sm font-medium mb-1">
            Tashkilot <span className="text-[var(--ms-text-destructive)]">*</span>
          </label>
          <NativeSelect
            id="se-org"
        ...
        <div>
          <label htmlFor="se-emp" className="block text-sm font-medium mb-1">
            Xodimlar <span className="text-[var(--ms-text-destructive)]">*</span>
          </label>
          <NativeSelect
            id="se-emp"
            className={SEL}
          ...
          {selectedEmployees.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2">
              {selectedEmployees.map((emp) => (
                <span
                  key={emp.id}
                  className="flex items-center gap-1 rounded-full bg-[var(--ms-brand-50)] text-[var(--ms-text-brand)] text-xs px-2.5 py-1"
                >
                  {emp.name}
                  <button
                    type="button"
