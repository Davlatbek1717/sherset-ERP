...
/**
 * /settings/customer-order-statuses — moysklad «Настроить статусы» parity
 * (for customer orders).
 *
 * Account-wide editable statuses (State rows, entityType="customerorder")
 * that drive the coloured «Статус» dropdown on the order editor + detail
 * header (e.g. «Текширилмаган»). Single-page CRUD with an inline create/edit
 * modal — each row is just name + colour + position, mirroring
 * /settings/task-types.
 *
...
// includes the live demo account's hues (red/orange/green/lime) so the
// defaults match what users already see.
...
export default function CustomerOrderStatusesPage() {
  const t = useTranslations('customer_order_statuses');
  ...
  const upsertMut = useMutation({
    ...
    onSuccess: () => {
      // Invalidate the whole `states` key so the order editor/detail dropdowns
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`co-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`co-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        testId="customer-order-statuses-page"
        title={t('title')}
        subtitle={t('subtitle')}
        moyskladToolbar
        onCreate={openCreate}
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `co-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
      ...
      >
        <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`co-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              htmlFor="co-status-position"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('position')}
            </label>
            <Input
              id="co-status-position"
              type="number"
              value={position}
              onChange={(e) => setPosition(Number(e.target.value) || 0)}
              data-test-id="co-status-position"
            />
          </div>
