...
/**
 * /settings/commission-report-statuses — moysklad «Настроить статусы» parity
 * (for «Выданный отчёт комиссионера»).
 *
 * Account-wide editable statuses (State rows, entityType="commissionreportout")
 * that drive the coloured «Статус» dropdown on the commission-report editor
 * header. Until the admin creates statuses here the report's «Статус» pill stays a
 * grey «Статус» (the FSM `state` + «Проведено» are separate). Single-page CRUD with
 * an inline create/edit modal — mirroring /settings/supply-statuses 1:1 (same State
 * REST surface).
 *
 * The /states list endpoint returns {items, nextCursor} (no total / search / sort),
 * so search is applied client-side; the status list is small.
 */
...
const ENTITY_TYPE = 'commissionreportout';
...
// matches the customer-order palette so all status sets read identically.
...
export default function CommissionReportStatusesPage() {
  const t = useTranslations('commission_report_statuses');
  ...
  const upsertMut = useMutation({
    ...
    onSuccess: () => {
      // Invalidate the whole `states` key so the commission editor's «Статус»
      // dropdown picks up the change too.
      ...
      cell: (row) => (
        <div className="flex items-center gap-2">
          <button
            ...
            data-test-id={`commission-report-status-archive-${row.id}`}
          >
          ...
          <button
            ...
            data-test-id={`commission-report-status-delete-${row.id}`}
          >
            {t('delete')}
          </button>
        </div>
  ...
  return (
    <>
      <ListView
        testId="commission-report-statuses-page"
        title={t('title')}
        subtitle={t('subtitle')}
        moyskladToolbar
        onCreate={openCreate}
        ...
        rows={rows}
        keyField="id"
        rowTestId={(row) => `commission-report-status-row-${row.id}`}
        total={rows.length}
        limit={LIMIT}
        hasNext={false}
        hasPrevious={false}
      ...
      <Modal
        ...
        widthClass="w-[480px]"
        testId="commission-report-status-modal"
        footer={
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <label
              htmlFor="commission-report-status-name"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('name')}
            </label>
            <Input
              id="commission-report-status-name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              data-test-id="commission-report-status-name"
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {DEFAULT_PALETTE.map((c) => (
                <button
                  ...
                  aria-label={c}
                  data-test-id={`commission-report-status-color-${c}`}
                />
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1">
            <label
              htmlFor="commission-report-status-position"
              className="font-medium text-[var(--ms-text-primary)] text-sm"
            >
              {t('position')}
            </label>
            <Input
              id="commission-report-status-position"
              type="number"
              value={position}
              onChange={(e) => setPosition(Number(e.target.value) || 0)}
              data-test-id="commission-report-status-position"
            />
          </div>
          ...
          {error && (
            <div
              ...
              data-test-id="commission-report-status-error"
            >
              {error}
            </div>
          )}
