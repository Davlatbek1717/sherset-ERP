...
export default function CashInListPage() {
  ...
  // (moysklad «Приходные ордера» parity: Группа контрагента / Договор /
  // Проект / Касса / Статус / Назначение платежа / Проведено /
  // Владелец-отдел / Когда изменен). Mirrors the payment-in gold standard.
  ...
  const { data, isLoading, error, refetch } = useQuery<ListResponse<CashInRow>>({
    queryKey: listQueryKey,
    queryFn: () => api.get<ListResponse<CashInRow>>(`/cash-in?${params.toString()}`),
  });

  const bulk = useBulkDocumentActions('cash-in', listQueryKey, { hasFSM: true });
  // moysklad parity (B3 audit, 2026-05-21): `updatedAt`/`owner` removed
  // from defaults (available via gear). `cashDesk` (Касса) is kept —
  // moysklad's cash list shows касса column by default.
  const cols = useColumnVisibility('cash-in', [
    'name',
    'moment',
    'organization',
    'agent',
  ...
  const columns: DataTableColumn<CashInRow>[] = [
    {
      key: 'name',
    ...
    {
      // moysklad «Приход» — receipt amount, right-aligned bold.
      key: 'sum',
      sortField: 'sumMinor',
  ...
  return (
    <>
      <ListView
        ...
        moyskladToolbar
        onRefresh={() => refetch()}
        onHelp={() => window.open('/help/cash-in', '_blank')}
        selectionCount={bulk.selectedIds.size}
        createHref="/cash-in/new"
        ...
        headerSlot={
          <InlineFilterPanel
            ...
            testId="cash-in-inline-filter"
          >
            {/* Inline filter panel — moysklad «Приходные ордера» parity,
               ordered to match the cashin filter reference
               (07-module/cashin/dom/00-clean-default.html): Период · Контрагент ·
               Группа контрагента · Договор · Владелец контрагента · Организация ·
               Касса · Проект · Статус · Назначение платежа · Проведено ·
               Владелец-сотрудник · Владелец-отдел · Сумма · Когда изменен.
               «Касса» (cashDeskId) is
               the cash-doc money source (no bank account). «Основание» is
               SKIPPED — it shares the `paymentPurpose` column with Назначение
               платежа (reference renders both via class="paymentPurpose"); a
               second filter would duplicate Назначение платежа. «Статья
               расходов» SKIPPED — CashIn is an income doc with no expenseItem
               column. «Кто изменил» SKIPPED — no updatedById column. */}
            {/* 1. Период */}
            ...
            {/* Владелец контрагента — the agent (Counterparty)'s owner employee
               (agent.ownerId). §4-grounded on the cashin filter capture
               (07-module/cashin/dom/00-clean-default.html, ordered Договор →
               Владелец контрагента → Организация). Distinct from «Владелец-
               сотрудник» (the cash order's own owner). NOTE: «Статья расходов»
               and «Счёт контрагента/организации» from moysklad's unified money
               filter do NOT apply — CashIn is an income doc (no expenseItem
               column) and cash docs use «Касса», a cash desk, not a bank
               account (no organizationAccountId). */}
