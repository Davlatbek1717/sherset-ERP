...
/**
 * HR Bo'limlar (Departments) — name-catalog CRUD.
 *
 * Simplest catalog list (mirrors HrRolesPage): a single «name» field, a
 * per-row employee count, and edit/delete. Soft-delete + a server-side
 * block when active employees still carry the name. See spec §5.2.
 */

import { hrDepartmentApi } from '@/lib/hr-api';
import type { HrDepartment } from '@/lib/hr-api';
import {
  Badge,
  Button,
  EmptyState,
...
export default function HrDepartmentsPage() {
  const t = useTranslations('pages.hrDepartments');
  ...
  const [editTarget, setEditTarget] = useState<HrDepartment | null>(null);
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery<HrDepartment[]>({
    queryKey: ['hr-departments'],
    queryFn: () => hrDepartmentApi.list(),
  });

  const deleteMut = useMutation({
    mutationFn: (id: string) => hrDepartmentApi.remove(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        ...
        <Button onClick={() => setCreateOpen(true)} data-test-id="hr-departments-create">
          + {t('create_button')}
        </Button>
      </div>
            ...
            <tbody>
              {rows.map((row) => (
                <tr
                  ...
                  data-test-id={`hr-department-row-${row.id}`}
                >
                  ...
                  <td className="px-3 py-2">
                    <div className="flex justify-end gap-1">
                      <Button
                        ...
                        data-test-id={`hr-department-edit-${row.id}`}
                      >
                        ✏️
                      </Button>
                      <Button
                        ...
                        data-test-id={`hr-department-delete-${row.id}`}
                      >
                        ❌
                      </Button>
                    </div>
      ...
      <DepartmentFormModal open={createOpen} onOpenChange={setCreateOpen} mode="create" />
      <DepartmentFormModal
        open={!!editTarget}
...
function DepartmentFormModal({
  ...
  const saveMut = useMutation({
    mutationFn: () => {
      if (mode === 'edit' && initial) {
        return hrDepartmentApi.update(initial.id, { name: name.trim() });
      }
      return hrDepartmentApi.create({ name: name.trim() });
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['hr-departments'] });
  ...
  return (
    <Modal
      ...
      widthClass="w-[480px]"
      testId="hr-department-modal"
      footer={
        <div className="flex justify-end gap-2">
          <Button variant="secondary" onClick={() => onOpenChange(false)}>
            {t('cancel')}
          </Button>
          <Button onClick={submit} disabled={saveMut.isPending} data-test-id="hr-department-save">
            {t('save')}
          </Button>
        </div>
      }
    >
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <Input
            id="hr-department-name"
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-auto"
            data-test-id="hr-department-name-input"
          />
        </div>
        ...
        {error && (
          <div
            ...
            role="alert"
            data-test-id="hr-department-modal-error"
          >
            {error}
          </div>
        )}
