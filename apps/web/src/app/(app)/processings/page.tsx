...
export default function ProcessingsPage() {
  ...
  // (MultiCombobox), holding the picked {id,label} pairs; on the wire it goes
  // out as `organizationIds` CSV. Was a single-select CatalogPicker modal.
  ...
  const bulk = useBulkDocumentActions('processings', listQueryKey, {
    ...
    hasBulkPrint: true,
    onMassEditClick: (ids) => {
      stashBulkEdit({ entity: 'processings', ids, from: '/processings' });
  ...
  const columns: DataTableColumn<ProcessingRow>[] = [
    ...
    // moysklad #processing list (Технологические операции) has NO «Статус»
    // column (LIVE-grounded 2026-06-29: default grid = №·Время·Организация·
    // Технологическая карта·Объём производства·Себестоимость·Отправлено·
  ...
  const filterPanel = (
    ...
    >
      ...
      {/* 4. Организация — moysklad-parity inline multi-select checkbox dropdown
