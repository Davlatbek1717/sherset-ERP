...
function getDefaultSalePrice(
  ...
  return resolveDefaultSalePrice(s.salePrices, defaultPriceTypeId, rates);
}
...
export default function ServicesPage() {
  ...
  // moysklad parity (D4 audit, 2026-05-28): additional filter state from
  // ProductFilterSchema. Services share the products filter backend
  // (kind=service); fields mirror the products D2 panel MINUS Тип товара
  // (fixed by kind=service in the query) and MINUS Тип учёта (services
  // are not stock-tracked — TrackingType applies to physical goods).
  // country = «Страна» (ISO2, normalised upper case in URL);
  // ownerId = «Владелец»; belowMinimum = «Ниже минимума» (tri-state —
  // '' = ignore, 'true' / 'false'). Captured 04-module reference DOM was
  // unreliable on prior audits (mislabeled), so order falls back to the
  // well-known moysklad set: folder → supplier → country → owner →
  // archived → belowMinimum.
  ...
  const { data, isLoading, error, refetch } = useQuery<ServiceListResponse>({
    queryKey: listQueryKey,
    queryFn: () => api.get<ServiceListResponse>(`/products?${params.toString()}`),
  });
  ...
  const cols = useColumnVisibility('services', ['name', 'code', 'folder', 'vat', 'price']);
  const colWidths = useColumnWidths('services');

  // moysklad's service list uses moyskladToolbar + Фильтр panel chrome
  ...
  const columns: DataTableColumn<Service>[] = [
    ...
    },
    {
      ...
      width: '140px',
      sortable: true,
      cell: (s) => <span className="text-[var(--ms-text-muted)] text-xs">{s.code ?? '—'}</span>,
      cellText: (s) => s.code ?? '',
    },
    {
      key: 'folder',
      ...
      cell: (s) => (
        <span className="text-[var(--ms-text-muted)] text-xs">
          {s.productFolder?.pathName ?? s.productFolder?.name ?? '—'}
        </span>
      ),
      cellText: (s) => s.productFolder?.pathName ?? s.productFolder?.name ?? '',
    },
    {
      key: 'vat',
      header: tFields('vat'),
      align: 'right',
      width: '80px',
      cell: (s) => <span className="text-sm">{s.vat != null ? `${s.vat}%` : '—'}</span>,
      cellText: (s) => (s.vat != null ? `${s.vat}%` : ''),
    },
    {
      key: 'price',
      ...
      width: '160px',
      cell: (s) => {
        const price = getDefaultSalePrice(s, defaultId, rates);
        return (
  ...
  return (
    <>
      <ListView
        ...
        headerSlot={
          /* Inline filter panel — moysklad-parity for the Услуги master
             list. Field order mirrors moysklad's Фильтр panel for product-
             kind views (D4 audit, 2026-05-28; 04-module reference DOM
             was unreliable on prior audits — D1/D2/D3 mislabels — so
             order falls back to the well-known moysklad set MINUS Тип
             товара (fixed by kind=service) and MINUS Тип учёта (services
             don't carry markirovka):
          ...
          >
            ...
            {/* 3. Страна — ISO 3166-1 alpha-2 (e.g. UZ, CN, RU). Free
                 text; only 2-char inputs are forwarded to the API. The
                 backend schema upper-cases the value so case doesn't
                 matter here. */}
            ...
            {/* 6. Ниже минимума — tri-state. Empty = ignore;
                 ...
                 Most services will never trigger true (stock typically
                 zero / not tracked), but the column exists on Product
                 and the filter is harmless for services. */}
