...
function formFromData(d: CashOutDetail): FormState {
  return {
    agentId: d.agent.id,
    ...
    operations: d.operations.map((op) => ({
      _uid: op.id,
      targetId: op.invoiceInId,
      targetLabel: op.invoiceIn?.name ?? '—',
      targetHint: op.invoiceIn ? `${op.invoiceIn.state}` : '',
...
export default function CashOutDetailPage() {
  ...
  const detailNav = useDetailNavigation('cash-out', id);
  ...
  const { data, isLoading } = useQuery<CashOutDetail>({
    queryKey: ['cash-out', id],
    queryFn: () => api.get(`/cash-out/${id}`),
  });
  ...
  const onConflict = useConflictReload(['cash-out', id], () => setForm(null));

  const transitionMut = useApiMutation({
    mutationFn: (target: string) => api.post(`/cash-out/${id}/transitions/${target}`, {}),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['cash-out', id] });
      qc.invalidateQueries({ queryKey: ['cash-out'] });
  ...
  const saveMut = useSaveMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        operations: form.operations.map((op) => ({
          ...
          invoiceInId: op.targetId,
          amountMinor: op.amountMinor,
        })),
        attributes: form.attributes,
      };
      return api.patch(`/cash-out/${id}`, payload);
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['cash-out', id] });
      qc.invalidateQueries({ queryKey: ['cash-out'] });
    ...
    onError: (err: Error) => {
      // An optimistic-lock 409 is owned by the reload dialog (onConflict) — keep
      // it OUT of the inline banner so the user sees the conflict prompt, not a
      // raw message. The local saveError state stays empty for this case.
  ...
  const cloneMut = useApiMutation({
    mutationFn: () => api.post<{ id: string }>(`/cash-out/${id}/clone`, {}),
    onSuccess: (cloned) => router.push(`/cash-out/${cloned.id}`),
  });

  const deleteMut = useApiMutation({
    mutationFn: () => api.delete(`/cash-out/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['cash-out'] });
  ...
  return (
    <div
      ...
      data-test-id="cash-out-detail-page"
    >
      <DetailToolbar
        isDirty={isDirty}
      ...
      />
      <DetailHeader
        titlePrefix={tDetailTitles('cash_out')}
        name={data.name}
        moment={data.moment}
      ...
      <main className="flex-1 px-4 py-4">
        ...
        <DocumentMetaPanel>
          ...
          <DocumentMetaRow>
            {/* «Статья расходов» — expense item (moysklad parity). Editable on a
               draft; persisting it is what makes the list filter live. Cash-out
        ...
        <div className="mt-6">
          <DetailContentTabs
            ...
            entityId={data.id}
            relatedGroups={[]}
            filesSlot={<AttachmentsSection entity="CashOut" entityId={data.id} />}
        ...
        <div className="mt-6 flex flex-col gap-3">
          <DocumentTasksSection entity="CashOut" entityId={data.id} />
        </div>
