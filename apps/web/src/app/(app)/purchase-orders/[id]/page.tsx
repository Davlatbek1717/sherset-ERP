...
export default function PurchaseOrderDetailPage() {
  ...
  const printForm = (templateId?: string) => {
    // moysklad «Открыть в браузере» — open the form PDF in a NEW TAB (the user
    // presses «Печать» there), not a save-to-disk download.
    void api
      .postOpenInBrowser('/purchase-orders/bulk-print', {
        ids: [data.id],
  ...
  const kitPrint = (templateIds: Array<string | null>) => {
    ...
    void api
      .postOpenInBrowser('/purchase-orders/kit-print', { ids: [data.id], templateIds })
      .then(() => qc.invalidateQueries({ queryKey: ['purchase-order', id] }));
  };
  ...
  const printMenuItems: CreateMenuItem[] = [
    ...
    // The standard built-in «Заказ поставщику» form — opens the HTML print view.
    {
      ...
      onSelect: () => window.open(`/print/purchase-order/${data.id}`, '_blank'),
    },
  ...
  const onApplicableChange = (['cancelled', 'closed'] as readonly string[]).includes(data.state)
    ? undefined
    : (next: boolean) => transitionMut.mutate(next ? 'confirm' : 'unconfirm');
    ...
    >
      <div className="w-full max-w-[1300px]">
        <DetailToolbar
          ...
          // «Отправить» — same as the /new editor. Each opens THIS order's PDF in a
          // new tab (printForm → postOpenInBrowser). Data-driven: none when the
          // account has no custom forms.
          trailingSlot={(printForms ?? []).map((f) => (
            <Button
              ...
              // matching supplies/new + purchase-orders/new. Owner request.
            ...
            <DetailContentTabs
              ...
              relatedSlot={
                <RelatedDocsTab
                  ...
                  // modal (pre-scoped to this order's agent/org/store), manual
                  // links, unlink and the «?link=new» auto-open hand-off.
                  linkable={{
