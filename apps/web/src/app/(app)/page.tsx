...
// ---------------------------------------------------------------------------
// Sotuv KPI cards — the daily kassa summary (Sherset custom). One backend
// call (/reports/sotuv-dashboard) feeds every card: income split (sotuvdan /
// qarzdan), profit chain, debt ledger snapshot, warehouse totals, payment
// method breakdown, and the per-cashier table. Auto-refreshes every 30s.
// ---------------------------------------------------------------------------
...
function SotuvKpiSection() {
  const { data, isLoading } = useQuery<SotuvDashboardResult>({
    queryKey: ['sotuv-dashboard'],
    queryFn: () => api.get<SotuvDashboardResult>('/reports/sotuv-dashboard'),
    staleTime: 30_000,
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Bugungi holat (snapshot) row */}
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <KpiCard
          icon={Users}
          label="Mijoz qarzlari (bugungi)"
          primary={v(data?.customerDebtMinor)}
        />
        <KpiCard icon={User} label="Shaxsiy qarzlar (bugungi)" primary={v(data?.ourDebtMinor)} />
        <KpiCard
          icon={Package}
          label="Tovarlar (bugungi)"
          primary={isLoading ? '…' : `${(data?.productCount ?? 0).toLocaleString('uz-UZ')} ta`}
        />
        <KpiCard
          icon={Warehouse}
          label="Ombor qiymati (bugungi)"
