...
function SalesSection({
  ...
  return (
    <section>
      ...
      <div className="pt-3">
        {/* Mobile: the 4 fixed-width pills (~400px) drove page overflow — the
            bar goes full-width with equal flexible segments below `md`. */}
        <div className="inline-flex max-md:flex max-md:w-full rounded-[var(--ms-radius-default)] border border-[var(--ms-border-default)] bg-[var(--ms-bg-muted)] p-0.5">
          {PERIODS.map((p) => (
            <button
              key={p}
              type="button"
              ...
              className={`min-w-[72px] max-md:min-w-0 max-md:flex-1 max-md:px-2 rounded-[calc(var(--ms-radius-default)-2px)] px-5 py-1.5 font-medium text-sm transition-colors ${
                period === p
