...
/**
 * Move list — "Изменить ▾" bulk-actions dropdown — moysklad parity.
 *
 * Source-of-truth: docs/moysklad-reference/moves/states/metadata.json
 * (captured 2026-05-30 via `pnpm capture-moysklad moves`).
 *
 ...
 *
 * Move's FSM uses verb-style transition slugs (`post` / `unpost`); see
 * apps/api/src/modules/move/move.schema.ts.
 */
...
export interface MoveBulkActionsDropdownProps {
...
export function MoveBulkActionsDropdown({
  selectedIds,
  listQueryKey,
  onClearSelection,
  postedCount = 0,
  onMassEdit,
}: MoveBulkActionsDropdownProps) {
  ...
  const bulkDelete = useApiMutation({
    mutationFn: async (rowIds: string[]) =>
      api.post<BulkResult>('/moves/bulk-delete', { ids: rowIds }),
    onSuccess: invalidate,
  });

  const bulkTransition = useApiMutation({
    mutationFn: async (input: { ids: string[]; target: string }) =>
      api.post<BulkResult>('/moves/bulk-transition', input),
    onSuccess: invalidate,
  });

  const bulkClone = useApiMutation({
    mutationFn: async (rowIds: string[]) => {
      await Promise.allSettled(rowIds.map((id) => api.post(`/moves/${id}/clone`, {})));
    },
    onSuccess: invalidate,
  });
  ...
  return (
    <DropdownMenu
      trigger={
        // moysklad parity (live, climart #move): «Изменить» opens at
        // 0-selection too — the menu shows with every item disabled (matches
        // the products `5cad088c` fix). Only `isPending` blocks the trigger.
        <Button variant="secondary" disabled={isPending}>
          {t('trigger')}
          <Icons.down className="h-4 w-4" />
        </Button>
      }
      testId="move-bulk-actions-dropdown"
    >
      <DropdownMenu.Item
        onSelect={handleDelete}
        destructive
        disabled={!hasSelection || isPending}
        testId="move-bulk-action-delete"
      >
        {t('delete')}
      </DropdownMenu.Item>
      <DropdownMenu.Item
      ...
      <DropdownMenu.Item
        ...
        testId="move-bulk-action-confirm"
      >
        {t('confirm')}
      </DropdownMenu.Item>
      <DropdownMenu.Item
        ...
        testId="move-bulk-action-unconfirm"
      >
        {t('unconfirm')}
      </DropdownMenu.Item>
      <DropdownMenu.Item disabled testId="move-bulk-action-merge">
        {t('merge')}
      </DropdownMenu.Item>
    </DropdownMenu>
  );
