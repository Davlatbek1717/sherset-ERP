...
import { formatMoney, Input } from '@moysklad/ui';
...
const FIELD_COLORS: Record<ActiveField, { border: string; bg: string; icon: string; dot: string }> = {
  cash:     { border: 'border-emerald-400', bg: 'bg-emerald-50',  icon: 'bg-emerald-500',  dot: 'bg-emerald-500' },
  card:     { border: 'border-blue-400',    bg: 'bg-blue-50',     icon: 'bg-blue-500',     dot: 'bg-blue-500' },
  terminal: { border: 'border-purple-400',  bg: 'bg-purple-50',   icon: 'bg-purple-500',   dot: 'bg-purple-500' },
};
...
export function RasmiyashtirishModal({
  ...
  const createMut = useMutation({
    mutationFn: (body: { name: string; phone: string | null; companyType: string; tags: string[] }) =>
  ...
  const canConfirm = !loading && totalPaid > 0n && (totalPaid >= sumMinor || (debtMinor > 0n && agent !== null));
  ...
  // ── Numpad handlers ───────────────────────────────────────────────────────
  const setActive =
    activeField === 'cash' ? setCashInput :
    activeField === 'card' ? setCardInput :
    setTerminalInput;
  ...
  const handleQuick = (amount: bigint) => {
    const cur =
      activeField === 'cash' ? cashMinor :
      activeField === 'card' ? cardMinor :
      terminalMinor;
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(96vw,42rem)] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-[var(--ms-bg-surface)] shadow-2xl outline-none flex flex-col max-h-[92dvh]">
          ...
          <div className="shrink-0 bg-[var(--ms-bg-app)] px-6 py-4 border-b border-[var(--ms-border)] rounded-t-2xl">
            <div className="flex items-start justify-between">
              ...
              <Dialog.Close asChild>
                <button type="button" className="flex h-8 w-8 items-center justify-center rounded-lg text-[var(--ms-text-muted)] hover:bg-[var(--ms-bg-hover)]">
                  <X className="h-4 w-4" />
                </button>
              </Dialog.Close>
            </div>
          ...
          <div className="flex flex-1 min-h-0">
            ...
            <div className="flex w-60 shrink-0 flex-col border-r border-[var(--ms-border)]">
              ...
              <div className="flex-1 overflow-y-auto flex flex-col">
                ...
                <div className="px-4 pt-4 pb-3 border-b border-[var(--ms-border)]">
                    <div className="mb-2">
                      <span className="text-[10px] text-[var(--ms-text-muted)] italic">Mijoz (ixtiyoriy)</span>
                    </div>
                    {agent ? (
                      <div className="flex items-center gap-2 rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-2">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-sm text-[var(--ms-text-primary)] truncate">{agent.name}</span>
                            {(() => { const b = cpTypeBadge(agent); return b ? <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold ${b.cls}`}>{b.label}</span> : null; })()}
                          </div>
                          {agent.phone && <div className="text-xs text-[var(--ms-text-muted)]">{agent.phone}</div>}
                        </div>
                        <button type="button" onClick={() => setAgent(null)} className="shrink-0 text-xs text-[var(--ms-text-muted)] hover:text-red-500">✕</button>
                      </div>
                    ) : creating ? (
                      <div className="flex flex-col gap-2">
                        ...
                        <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Ism Familiya / Tashkilot nomi" autoFocus />
                        <Input value={newPhone} onChange={(e) => setNewPhone(e.target.value)} placeholder="+998 __ ___ __ __" inputMode="tel" />
                        <div className="flex gap-2">
                          ...
                          <button type="button" onClick={() => setCreating(false)} className="rounded-lg border border-[var(--ms-border)] px-3 py-1.5 text-xs text-[var(--ms-text-muted)]">Bekor</button>
                        </div>
                      </div>
                    ) : (
                      <div className="rounded-xl border border-[var(--ms-border)] overflow-hidden">
                        ...
                        <div className="max-h-28 overflow-y-auto">
                          {cpLoading ? (
                            <p className="py-3 text-center text-xs text-[var(--ms-text-muted)]">...</p>
                          ) : (cpData?.items.length ?? 0) === 0 ? (
                            <p className="py-3 text-center text-xs text-[var(--ms-text-muted)]">Topilmadi</p>
                          ) : cpData?.items.map((c) => {
                            const badge = cpTypeBadge(c);
                            return (
                              <button type="button" key={c.id} onClick={() => setAgent(c)} className="flex w-full items-center gap-2 border-b border-[var(--ms-border)] last:border-0 px-3 py-2 text-left hover:bg-[var(--ms-bg-hover)]">
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-1.5">
                                    <span className="font-medium text-xs truncate">{c.name}</span>
                                    {badge && (
                                      <span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold ${badge.cls}`}>
                                        {badge.label}
                                      </span>
                                    )}
                                  </div>
                                  {c.phone && <span className="text-[10px] text-[var(--ms-text-muted)]">{c.phone}</span>}
                                </div>
                              </button>
                            );
                          })}
                        ...
                        <button
                          type="button"
                          onClick={() => { setCreating(true); setNewType('oddiy'); }}
                ...
                <div className="flex flex-col gap-2 p-4">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-[var(--ms-text-muted)]">To'lov turi</p>
                  ...
                  >
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${activeField === 'cash' ? 'bg-emerald-500' : 'bg-[var(--ms-bg-input)]'}`}>
                      <Banknote className={`h-4 w-4 ${activeField === 'cash' ? 'text-white' : 'text-[var(--ms-text-muted)]'}`} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-[var(--ms-text-muted)]">Naqd</div>
                      <div className={`font-bold tabular-nums leading-tight text-sm ${cashMinor > 0n ? 'text-[var(--ms-text-primary)]' : 'text-[var(--ms-text-muted)]'}`}>
                        {cashMinor > 0n ? formatMoney(cashMinor) : '—'}
                      </div>
                    </div>
                    {activeField === 'cash' && <div className="h-2 w-2 shrink-0 rounded-full bg-emerald-500" />}
                  </button>
                  ...
                  >
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${activeField === 'card' ? 'bg-blue-500' : 'bg-[var(--ms-bg-input)]'}`}>
                      <CreditCard className={`h-4 w-4 ${activeField === 'card' ? 'text-white' : 'text-[var(--ms-text-muted)]'}`} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-[var(--ms-text-muted)]">Karta</div>
                      <div className={`font-bold tabular-nums leading-tight text-sm ${cardMinor > 0n ? 'text-[var(--ms-text-primary)]' : 'text-[var(--ms-text-muted)]'}`}>
                        {cardMinor > 0n ? formatMoney(cardMinor) : '—'}
                      </div>
                    </div>
                    {activeField === 'card' && <div className="h-2 w-2 shrink-0 rounded-full bg-blue-500" />}
                  </button>
                  ...
                  >
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${activeField === 'terminal' ? 'bg-purple-500' : 'bg-[var(--ms-bg-input)]'}`}>
                      <Monitor className={`h-4 w-4 ${activeField === 'terminal' ? 'text-white' : 'text-[var(--ms-text-muted)]'}`} />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[10px] font-semibold uppercase tracking-wide text-[var(--ms-text-muted)]">Terminal</div>
                      <div className={`font-bold tabular-nums leading-tight text-sm ${terminalMinor > 0n ? 'text-[var(--ms-text-primary)]' : 'text-[var(--ms-text-muted)]'}`}>
                        {terminalMinor > 0n ? formatMoney(terminalMinor) : '—'}
                      </div>
                    </div>
                    {activeField === 'terminal' && <div className="h-2 w-2 shrink-0 rounded-full bg-purple-500" />}
                  </button>
                </div>
                  ...
                  {debtMinor > 0n ? (
                    agent ? (
                      <div className="rounded-xl bg-red-50 border border-red-300 px-3 py-3 text-center">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-red-500 mb-0.5">Qarz</p>
                        <p className="text-xl font-bold tabular-nums text-red-700 leading-none">{formatMoney(debtMinor)}</p>
                      ...
                      <div className="rounded-xl bg-orange-50 border border-orange-200 px-3 py-3 text-center">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-orange-500 mb-0.5">Qoldi</p>
                        <p className="text-xl font-bold tabular-nums text-orange-700 leading-none">{formatMoney(debtMinor)}</p>
                        <p className="mt-1 text-[10px] text-orange-400">Qarzga berish uchun mijoz tanlang</p>
                      </div>
                    )
                  ) : change > 0n ? (
                    <div className="rounded-xl bg-emerald-50 border border-emerald-300 px-3 py-3 text-center">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-500 mb-0.5">Qaytim</p>
                      <p className="text-2xl font-bold tabular-nums text-emerald-700 leading-none">{formatMoney(change)}</p>
                    </div>
            ...
            <div className="flex flex-1 flex-col gap-2 p-4 bg-[var(--ms-bg-app)]">
              ...
              <div className={`rounded-xl border-2 px-4 py-2 transition-colors ${colors.border} bg-white`}>
                ...
                <input
                  ...
                  inputMode="decimal"
                  min="0"
                  value={activeField === 'cash' ? cashInput : activeField === 'card' ? cardInput : terminalInput}
