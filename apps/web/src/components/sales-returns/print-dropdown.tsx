...
/**
 * Sales-return list — "Печать ▾" print-templates dropdown.
 *
 * Item set sibling-mirrored from the document-list family; return templates
 * capture is empty (docs/moysklad-reference/print-templates/salesreturn/
 ...
 *   1. Список возвратов покупателей (enabled — list export)
...
export interface SalesReturnPrintDropdownProps {
...
export function SalesReturnPrintDropdown({
  onExportList,
  selectedIds,
}: SalesReturnPrintDropdownProps) {
  const t = useTranslations('print_menu');
  const tReturn = useTranslations('print_menu_salesreturn');
  ...
  return (
    <DropdownMenu
      ...
      testId="sales-return-print-dropdown"
    >
      <DropdownMenu.Item
        onSelect={() => onExportList?.()}
        disabled={!onExportList}
        testId="sales-return-print-list-export"
      >
        {tReturn('list_export')}
      </DropdownMenu.Item>
