...
/**
 * Purchase-return list — "Изменить ▾" bulk-actions dropdown — moysklad parity.
 *
 ...
 *
 * PurchaseReturn's FSM uses verb-style transition slugs (`post` / `unpost`);
 * see apps/api/src/modules/purchase-return/purchase-return.schema.ts.
 */
...
export interface PurchaseReturnBulkActionsDropdownProps {
...
export function PurchaseReturnBulkActionsDropdown({
  selectedIds,
  listQueryKey,
  onClearSelection,
  postedCount = 0,
  onMassEdit,
}: PurchaseReturnBulkActionsDropdownProps) {
  ...
  const bulkDelete = useApiMutation({
    mutationFn: async (rowIds: string[]) =>
      api.post<BulkResult>('/purchase-returns/bulk-delete', { ids: rowIds }),
    onSuccess: invalidate,
  });

  const bulkTransition = useApiMutation({
    mutationFn: async (input: { ids: string[]; target: string }) =>
      api.post<BulkResult>('/purchase-returns/bulk-transition', input),
    onSuccess: invalidate,
  });
  ...
  const bulkClone = useApiMutation({
    mutationFn: async (rowIds: string[]) => {
      await Promise.allSettled(rowIds.map((id) => api.post(`/purchase-returns/${id}/clone`, {})));
    },
    onSuccess: invalidate,
  });
  ...
  return (
    <DropdownMenu
      ...
      testId="purchase-return-bulk-actions-dropdown"
    >
      <DropdownMenu.Item
        onSelect={handleDelete}
        destructive
        disabled={!hasSelection || isPending}
        testId="purchase-return-bulk-action-delete"
      >
        {t('delete')}
      </DropdownMenu.Item>
      <DropdownMenu.Item
        onSelect={handleCopy}
        disabled={!hasSelection || isPending}
        testId="purchase-return-bulk-action-copy"
      >
        {t('copy')}
      </DropdownMenu.Item>
      <DropdownMenu.Item
        ...
        testId="purchase-return-bulk-action-mass-edit"
      >
        {t('mass_edit')}
      </DropdownMenu.Item>
      <DropdownMenu.Item
        ...
        testId="purchase-return-bulk-action-confirm"
      >
        {t('confirm')}
      </DropdownMenu.Item>
      <DropdownMenu.Item
        ...
        testId="purchase-return-bulk-action-unconfirm"
      >
        {t('unconfirm')}
      </DropdownMenu.Item>
      <DropdownMenu.Item disabled testId="purchase-return-bulk-action-merge">
        {t('merge')}
      </DropdownMenu.Item>
    </DropdownMenu>
  );
