/**
 * ISO 3166-1 country reference (alpha-2 code + Russian short name).
 *
 * Used by the «Страна» combo on the product create/detail forms. moysklad shows
 * a searchable country dropdown; our product model stores the **ISO alpha-2
 * code** (`country` field, validated `/^[A-Z]{2}$/`), so the combo's VALUE is the
 * code and the label is the Russian name. The stored code is unambiguous, so the
 * display spelling is non-critical — search matches both the name and the code.
 *
 * This is canonical, stable reference data (ISO 3166-1), not a moysklad-specific
 * field label — it doesn't need per-render DOM grounding (CLAUDE.md §4 covers
 * field LABELS, not reference-list contents).
 */

export interface CountryOption {
  /** ISO 3166-1 alpha-2 code (what we store). */
  code: string;
  /** Russian short name (display). */
  name: string;
}

/** Full ISO 3166-1 set (source order = by code; the combo sorts by name). */
...
/** De-duplicated by code (VG appears twice historically), sorted by Russian name. */
export const COUNTRIES: CountryOption[] = Array.from(
  new Map(RAW.map((c) => [c.code, c])).values(),
).sort((a, b) => a.name.localeCompare(b.name, 'ru'));

/** O(1) code → name lookup for rendering a stored value. */
