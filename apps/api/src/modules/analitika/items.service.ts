...
export class ItemsService {
   ...
   *
   * Two-phase strategy keeps DB load bounded across sort modes:
   *   1. Resolve the matching product IDs (filtered, but only IDs + simple
   *      fields — cheap).
   *   2. Compute per-product aggregates (purchased/sold/stock/last-supply)
   *      for the resolved set, sort + filter (lowStock) in JS, then paginate.
   *
   * For sort='name' or 'code' (DB-sortable), we still fetch IDs first because
   * `lowStock` and `noPartner`/`onlyInCart` filters need the aggregates anyway.
   * Bounded by `MAX_PRODUCTS_PER_QUERY` for safety; realistic catalogues fit.
   */
  async list(accountId: string, raw: unknown): Promise<ItemsResponse> {
    ...
    // Phase 1: candidate IDs + lightweight fields for sort 'name'/'code'.
    ...
    // Phase 2: aggregates for the candidate set.
    ...
    // Filter: lowStock (computed after aggregates).
    ...
    return { items, total, page: filter.page, pageSize: filter.pageSize, totalPages };
  }
  ...
  async stats(accountId: string, raw: unknown): Promise<ItemsStats> {
    ...
    const noPartnerCount = products.filter((p) => !p.supplierId).length;
    ...
    return { totalItems: products.length, lowStockCount, noPartnerCount };
  }
...
const MAX_PRODUCTS_PER_QUERY = 10_000;
