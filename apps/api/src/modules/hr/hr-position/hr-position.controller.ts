...
import { CreateHrPositionSchema, UpdateHrPositionSchema } from './hr-position.schema.js';
import { HrPositionService } from './hr-position.service.js';

@Controller('hr/positions')
@UseGuards(JwtAuthGuard, HrPermissionGuard)
export class HrPositionController {
  constructor(@Inject(HrPositionService) private readonly svc: HrPositionService) {}
  ...
  async create(@CurrentUser() user: AuthenticatedUser, @Body() body: unknown) {
    return this.svc.create(user.accountId, CreateHrPositionSchema.parse(body));
  }
  ...
  async update(
    ...
    return this.svc.update(user.accountId, id, UpdateHrPositionSchema.parse(body));
  }
