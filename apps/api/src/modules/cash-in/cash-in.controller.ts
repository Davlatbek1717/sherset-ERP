...
export class CashInController {
  constructor(@Inject(CashInService) private readonly svc: CashInService) {}

  @Get()
  @RequirePermission({ entity: 'cashin', action: 'view' })
  ...
  @RequirePermission({ entity: 'cashin', action: 'approve' })
  async transition(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') id: string,
  ...
  @Delete(':id')
  @RequirePermission({ entity: 'cashin', action: 'delete' })
