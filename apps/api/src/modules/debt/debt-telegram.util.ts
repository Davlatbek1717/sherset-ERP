 ...
 *     bitta `*qalin*` (ikkita EMAS), `__tagliq__` (underline — DEFOLT
 *     parserda umuman yo'q), `~kesilgan~`, `` `kod` ``. Summalar endi
 *     `__tagliq__` (underline), aloqa-blok label'lari `*qalin*`.
...
// summa __tagliq__) → 📞💳👨‍💻 aloqa/karta bloki (label'lar *qalin*) →
 ...
 * (2026-07-20b: label'lar *qalin* qilindi — foydalanuvchi talabi bilan.)
 */
...
export function debtIssuedMessage(args: {
  ...
  const lines = [
    ...
    '',
    `🧾 Sizga __${fmtSom(args.totalMinor)} so'm__ miqdorida qarz rasmiylashtirildi.`,
  ];
...
export function paymentMessage(args: {
  ...
  return [
    `Assalomu alaykum, hurmatli ${mdSafe(args.name)}!`,
    '',
    `💵 __${fmtSom(args.amountMinor)} so'm__ to'lovingiz qabul qilindi, rahmat!`,
    `Qolgan qarzingiz: __${fmtSom(args.remainingMinor)} so'm__.`,
    '',
    ...contactBlock(),
    '',
    'SHERSET jamoasi!',
...
export function debtClosedMessage(args: { name: string; amountMinor: bigint }): string {
  return [
    `Assalomu alaykum, hurmatli ${mdSafe(args.name)}!`,
    '',
    `✅ __${fmtSom(args.amountMinor)} so'm__ to'lovingiz qabul qilindi. Qarzingiz to'liq yopildi!`,
    '',
...
export function paymentReversedMessage(args: {
  ...
  return [
    `Assalomu alaykum, hurmatli ${mdSafe(args.name)}!`,
    '',
    `⚠️ __${fmtSom(args.amountMinor)} so'm__ to'lov yozuvi texnik xatolik tufayli bekor qilindi.`,
    `Joriy qarzingiz: __${fmtSom(args.remainingMinor)} so'm__.`,
    '',
    ...contactBlock(),
    '',
    'SHERSET jamoasi!',
 ...
 * ko'rinardi). Summa __tagliq__ (underline) qilinadi — GramJS'ning
...
export function reminderMessage(args: { name: string; remainingMinor: bigint }): string {
  return [
    `Assalomu alaykum, hurmatli ${mdSafe(args.name)}!`,
    '',
    `✅ Eslatib o'tamiz, Sizning __${fmtSom(args.remainingMinor)} so'm__ miqdorida to'lanmagan qarzingiz mavjud. Iltimos, kelishilgan muddatda qarzdorlikni yopishingizni so'raymiz.`,
    '',
    ...contactBlock(),
    '',
