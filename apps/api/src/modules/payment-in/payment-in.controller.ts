...
@Controller('payments-in')
@UseGuards(JwtAuthGuard)
export class PaymentInController {
  constructor(@Inject(PaymentInService) private readonly payment: PaymentInService) {}
  ...
  /**
   * moysklad header «N из ВСЕГО ‹ ›» record navigator (mirror PO/CO). Deeper
   * path than `:id` so it is not shadowed.
   */
  @Get(':id/position')
  @RequirePermission({ entity: 'paymentin', action: 'view' })
  async position(@CurrentUser() user: AuthenticatedUser, @Param('id') id: string) {
    return this.payment.findPosition(user.accountId, id);
  }
  ...
  @RequirePermission({ entity: 'paymentin', action: 'approve' })
  async transition(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') id: string,
  ...
  @Delete(':id')
  @RequirePermission({ entity: 'paymentin', action: 'delete' })
