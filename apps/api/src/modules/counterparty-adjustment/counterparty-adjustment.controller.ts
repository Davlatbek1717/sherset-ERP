...
import { CounterpartyAdjustmentTransitionSchema } from './counterparty-adjustment.schema.js';
import { CounterpartyAdjustmentService } from './counterparty-adjustment.service.js';

@Controller('counterparty-adjustments')
@UseGuards(JwtAuthGuard)
export class CounterpartyAdjustmentController {
  constructor(
    @Inject(CounterpartyAdjustmentService)
    private readonly service: CounterpartyAdjustmentService,
    @Inject(DocPdfService) private readonly docPdf: DocPdfService,
  ) {}

  @Get()
  @RequirePermission({ entity: 'counterpartyadjustment', action: 'view' })
  ...
  @Post()
  @RequirePermission({ entity: 'counterpartyadjustment', action: 'create' })
  async create(@CurrentUser() user: AuthenticatedUser, @Body() body: unknown) {
    return this.service.create(user.accountId, user.sub, body);
  }

  @Patch(':id')
  @RequirePermission({ entity: 'counterpartyadjustment', action: 'update' })
  async update(
  ...
  @Delete(':id')
  @RequirePermission({ entity: 'counterpartyadjustment', action: 'delete' })
  ...
  @RequirePermission({ entity: 'counterpartyadjustment', action: 'approve' })
  async bulkTransition(@CurrentUser() user: AuthenticatedUser, @Body() body: unknown) {
    ...
    const validTarget = CounterpartyAdjustmentTransitionSchema.parse(target);
  ...
  async bulkPrint(
    ...
    for (const id of ids) {
      ...
      docs.push({
        title: 'Kontragent balans tuzatish',
        number: doc.name,
        date: doc.moment,
    ...
    reply
      .header(
        'Content-Disposition',
        `attachment; filename="counterparty-adjustments-${ids.length}.pdf"`,
      )
      .send(merged);
  }
}
