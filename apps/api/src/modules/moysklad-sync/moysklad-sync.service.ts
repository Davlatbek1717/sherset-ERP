...
export interface SyncStatus {
  ...
  result: Record<string, SyncCounts> | null;
  error: string | null;
}
...
export class MoyskladSyncService {
  ...
  run(accountId: string): SyncStatus {
    ...
    void this.execute(accountId, token, status).catch((e) => {
      status.state = 'error';
  ...
  private async fetchAll<T>(token: string, entity: string): Promise<T[]> {
    const rows: T[] = [];
    let offset = 0;
    for (;;) {
      const url = `${MS_BASE}/entity/${entity}?limit=${PAGE}&offset=${offset}`;
      ...
      for (let attempt = 0; attempt < RETRIES; attempt++) {
        try {
          ...
          if (!res.ok) throw new Error(`MoySklad ${entity} HTTP ${res.status}`);
  ...
  private async execute(accountId: string, token: string, s: SyncStatus): Promise<void> {
    ...
    s.stage = 'uom';
    s.progress = null;
    const uoms = await this.fetchAll<MsUom>(token, 'uom');
    const uomName = new Map<string, string>();
    for (const u of uoms) uomName.set(u.id, u.name);

    // 2 — product folders
    s.stage = 'folders';
    const folders = await this.fetchAll<MsFolder>(token, 'productfolder');
    ...
    // 4 — products
    s.stage = 'products';
    const products = await this.fetchAll<MsProduct>(token, 'product');
    ...
    const cps = await this.fetchAll<MsCounterparty>(token, 'counterparty');
