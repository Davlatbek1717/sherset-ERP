...
export class CashOutController {
  constructor(@Inject(CashOutService) private readonly svc: CashOutService) {}

  @Get()
  @RequirePermission({ entity: 'cashout', action: 'view' })
  ...
  @RequirePermission({ entity: 'cashout', action: 'approve' })
  async transition(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id') id: string,
  ...
  @Delete(':id')
  @RequirePermission({ entity: 'cashout', action: 'delete' })
