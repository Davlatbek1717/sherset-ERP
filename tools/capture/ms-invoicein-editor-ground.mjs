// READ-ONLY live grounding of the moysklad «Счёт поставщика» (invoicein) EDITOR.
// Grounds: (1) an EXISTING doc editor (populated) — full/meta/lower screenshots,
// ordered meta-field labels, toolbar buttons, tab strip; (2) the inline-vs-modal
// behaviour of ref fields — clicks Контрагент / Договор / a position-name cell and
// screenshots + DOM-classifies whether moysklad shows an INLINE dropdown
// (gwt-SuggestBox popup) or a MODAL dialog; (3) the NEW create form toolbar via the
...
const classifyPopup = () =>
  p.evaluate(() => {
    ...
    // A modal in moysklad = a large centered dialog with a Caption/title bar + overlay.
    const dialogs = [...document.querySelectorAll('.gwt-DialogBox, .modal, [role="dialog"]')].filter(
      vis,
    );
    // An inline suggest = a gwt popup attached just below the field (suggest list).
    const suggests = [
    ...
    return {
      ...
      dialogRect: sampleDialog
        ? (() => {
            const r = sampleDialog.getBoundingClientRect();
      ...
      popupRect: samplePopup
        ...
        : null,
      // verdict: a centered overlay dialog => MODAL; a small popup anchored below a field => INLINE
...
const clickFieldByLabel = async (label, file) => {
  try {
    // find the editable field cell to the right of the caption label
    ...
    // click ~ to the right of the label (the input area)
...
try {
  ...
  if (editHref) {
    ...
    await shot('11-editor-meta.png', { x: 0, y: 110, width: 1340, height: 470 });
    await shot('12-editor-lower.png', { x: 0, y: 560, width: 1680, height: 430 });

    // ordered meta-field caption labels
    ...
    out.editorToolbar = await p.evaluate(() => {
    ...
    out.positionColumns = await p.evaluate(() => {
      // the positions grid headers (lower half of the page)
      const heads = [...document.querySelectorAll('th, [role="columnheader"], .gwt-table-header td, .header td')]
        .filter((e) => {
          const r = e.getBoundingClientRect();
    ...
    out.headerHints = await p.evaluate(() => {
      ...
      return {
        ...
        hasZakaz: /Заказ поставщику/.test(body),
    ...
    out.refFieldBehaviour.kontragent = await clickFieldByLabel('Контрагент', '30-click-kontragent.png');
  ...
  // ---- NEW create form toolbar (open blank editor via list «Счет» button) ----
  await p.goto(`${base}#invoicein`, { waitUntil: 'domcontentloaded' });
  ...
  try {
    ...
    if (await createBtn.count()) {
      ...
      await shot('41-new-editor-meta.png', { x: 0, y: 110, width: 1340, height: 470 });
      out.newToolbar = await p.evaluate(() => {
        const btns = [...document.querySelectorAll('button, a.Button, .Button, .toolbar *')]
          .filter((e) => {
            const r = e.getBoundingClientRect();
            return r.top > 55 && r.top < 135 && r.width > 8;
      ...
      out.newMetaFields = await p.evaluate(() => {
        const labels = [];
        for (const el of document.querySelectorAll('td, div, span, label')) {
          const r = el.getBoundingClientRect();
          if (r.top < 110 || r.top > 600 || r.height > 40 || r.width < 24) continue;
          ...
          if (!t || t.length > 42 || el.children.length > 1) continue;
          if (/^[А-ЯA-Z][а-яёa-z.,()\/ -]+\*?$/.test(t) && t.length >= 3)
            labels.push({ t, y: Math.round(r.top), x: Math.round(r.left) });
        }
        const seen = new Set();
        return labels
          .filter((l) => (seen.has(l.t) ? false : seen.add(l.t)))
