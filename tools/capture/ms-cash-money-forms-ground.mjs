// READ-ONLY live grounding of moysklad's 3 remaining money-create forms:
//   cashin   = «Приходный ордер»  (cash IN — «+ Приход» 2nd form)
//   paymentout = «Исходящий платёж» (bank OUT — «+ Расход» 1st form)
//   cashout  = «Расходный ордер»  (cash OUT — «+ Расход» 2nd form)
// For each: open the FIRST existing doc editor (populated), screenshot full+meta+lower,
// capture ordered meta-field labels + toolbar + tab strip + header hints. NEVER clicks
// Сохранить/Удалить/Да — navigation + Escape only. Credentials from .env.local, never printed.
...
const headerHints = () =>
  p.evaluate(() => {
    ...
    return {
      ...
      hasPoluchatel: /Получатель/.test(body),
      hasOplachDocs: /Оплаченные документы/.test(body),
      hasSvyazDocs: /Связанные документы/.test(body),
    };
  });
...
async function groundForm(key, route, editMatch, idx) {
  ...
  if (info.editHref) {
    ...
    info.tabs = await p.evaluate(() => {
      const tabs = [...document.querySelectorAll('[role="tab"], .gwt-TabBar *, .tabs *')]
        .map((e) => (e.textContent || '').trim())
        .filter((t) => /Связанн|Файл|Задач|Оплач|Документ/i.test(t) && t.length < 30);
...
try {
  ...
  await groundForm('cashout', 'cashout', 'cashout/edit', 3);
} catch (e) {
  out.error = String(e).slice(0, 400);
}

writeFileSync(resolve(OUT, 'cash-money-forms-ground.json'), JSON.stringify(out, null, 2));
