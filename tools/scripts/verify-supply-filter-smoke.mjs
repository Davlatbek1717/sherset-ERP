// Reusable runtime smoke — supplies list moysklad «Приёмки» Фильтр parity field
// wired in 2026-06-12 (purchase-side sibling of the cash-in/cash-out conveyor).
// Run: node tools/scripts/verify-supply-filter-smoke.mjs
// (needs the dev api on :4000 + dev DB + seeded admin@demo.local). Proves, LIVE,
// that the newly-wired filter actually NARROWS GET /supplies — i.e. is not a
// dead «accepted-but-unapplied» param (the 11h dead-column class):
//   - «Владелец контрагента» → ?agentOwnerId=<empId>   (agent.ownerId nested),
//     distinct from «Владелец-сотрудник» (the supply's OWN ownerId).
...
//
// DELIBERATE ABSENCES (locked by apps/web/.../supplies-filter-fields.test.ts):
//   - «Кто изменил» — Supply has no `updatedById` column.
//   - «Общий доступ» — `Supply.shared` exists but is never written.
...
const TOKEN = 'ZZSUPFILT_'; // appears ONLY in description (search-narrowed)
const NAME = 'ZZDOC-SUPFILTER'; // doc name prefix (distinct from TOKEN + CP names)
...
let token = null;
const docIds = {}; // role -> supply.id
const idToRole = new Map();

/** GET /supplies?<query> → Set of role-suffixes of our test rows returned. */
async function query(qs) {
  const r = await fetch(`${API}/supplies?search=${TOKEN}&limit=200&${qs}`, {
    headers: { authorization: `Bearer ${token}` },
  });
  if (!r.ok) throw new Error(`GET /supplies?${qs} → ${r.status}`);
  const body = await r.json();
  const roles = new Set();
...
async function main() {
  ...
  // A store is REQUIRED to create a supply. Reuse a seeded store if one exists,
  // else create one.
  ...
  if (existingStore) {
    ...
    const store = await prisma.store.create({ data: { accountId, name: `${NAME} store` } });
    storeId = store.id;
    storeCreated = true;
  }
  check('setup: a store is available', !!storeId, storeId?.slice(0, 8));

  // Counterparties — vary (groupId, ownerId) to exercise the merged agent clause.
  const mkCp = (role, data) =>
    prisma.counterparty
      .create({ data: { accountId, name: `ZZCPSUP_${role}`, ...data } })
      .then((c) => {
        cpIds[role] = c.id;
        return c;
      });
  ...
  // Supplies for the agent/group filters — created directly via prisma (the
  ...
  // The TOKEN lives in `description` (a search-OR field) so `search=TOKEN`
  // narrows to exactly our rows.
  let seq = 0;
  ...
  check(
    `setup: default view shows all ${TOTAL} test supplies`,
    baseline.size === TOTAL,
...
async function cleanup() {
  ...
  if (ids.length) await prisma.supply.deleteMany({ where: { id: { in: ids } } });
