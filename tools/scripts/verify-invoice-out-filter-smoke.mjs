// Reusable runtime smoke — invoices-out list moysklad «Счета покупателям» Фильтр
// parity field wired in 2026-06-12 (SALES-side mirror of the invoices-in /
// supplies / cash-in / cash-out conveyor).
// Run: node tools/scripts/verify-invoice-out-filter-smoke.mjs
// (needs the dev api on :4000 + dev DB + seeded admin@demo.local). Proves, LIVE,
// that the newly-wired filter actually NARROWS GET /invoices-out — i.e. is not a
...
//
// DELIBERATE ABSENCES (locked by apps/web/.../invoices-out-filter-fields.test.ts):
//   - «Кто изменил» — InvoiceOut has no `updatedById` column.
//   - «Общий доступ» — `InvoiceOut.shared` exists but is never written.
...
const TOKEN = 'ZZINVOUTFILT_'; // appears ONLY in description (search-narrowed)
const NAME = 'ZZDOC-INVOUTFILTER'; // doc name prefix (distinct from TOKEN + CP names)
...
let token = null;
const docIds = {}; // role -> invoiceOut.id
const idToRole = new Map();

/** GET /invoices-out?<query> → Set of role-suffixes of our test rows returned. */
async function query(qs) {
  const r = await fetch(`${API}/invoices-out?search=${TOKEN}&limit=200&${qs}`, {
    headers: { authorization: `Bearer ${token}` },
  });
  if (!r.ok) throw new Error(`GET /invoices-out?${qs} → ${r.status}`);
  const body = await r.json();
  const roles = new Set();
...
async function main() {
  ...
  const mkCp = (role, data) =>
    prisma.counterparty
      .create({ data: { accountId, name: `ZZCPINVOUT_${role}`, ...data } })
      .then((c) => {
        cpIds[role] = c.id;
        return c;
      });
...
async function cleanup() {
  ...
  if (ids.length) await prisma.invoiceOut.deleteMany({ where: { id: { in: ids } } });
