...
| 12 | [ ] | 🟠 **QISMAN — 25 fayl qoldi (o'lchamli qarz)** | Raw-element ban 32 aybdor ko'rsatdi. Men **kiritgan** bittasi tuzatildi (`settings/sms/templates` xom `<textarea>` → DS `Textarea`; sahifa #138 da DS-migratsiyasidan oldingi checkout'dan tiklangan edi). Qolgan **25 fayl** = climart yuzalarining haqiqiy konvensiya-qarzi (`<select>` 5 · `<textarea>` 3 · radio 7 · `input:textnum` 10+). **Istisno registriga tashlash guardni o'ldiradi** — registr tor va har yozuvi asosli. Har fayl mos DS primitivini talab qiladi (vizual/xulq riski) → alohida sessiya ishi | 🟠 | |
